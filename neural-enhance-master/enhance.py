...
class Model(object):
    ...
    def setup_generator(self, input, config):
        ...
        concatenated_layer = ConcatLayer(
            [self.network['iter.1-A>'], self.network['iter.2-A>'], self.network['iter.3-A>'],
             self.network['iter.4-A>']])
        conv = ConvLayer(concatenated_layer, units, filter_size=(7, 7), pad=(3, 3), nonlinearity=None)

        self.network["generator_concatination"] = concatenated_layer
        self.network["after_concat_conv"] = conv
